  ...
  if (modalId == 'addEventModal') {
    ...
    form_addEventModal.forEach((input) => {
      if (input == 'ModaleventType_doc') {
        ...
        document
          .getElementById('container_eventfullblocked')
          .classList.remove('dark:border-red-900', 'bg-red-500');
        document.getElementById(input).checked = false;
...
function updateModal(data) {
  ...
  let d = data[row_id];
  let row_price = totalBookingPriceCal(
    d.Price,
    d.QtTraveller,
    d.Tax,
    d.Fee,
  ...
  if (ModalInStack('ListEventModal') && isNaN(parseFloat(row_id))) {
    ...
    document.getElementById('booking_startDay_' + row_id).innerText =
      getDayOfWeek(format_date(d.start));
    document.getElementById('booking_start_' + row_id).innerText = format_date(
      d.start,
      0,
      'DD/MM'
    );
    document.getElementById('booking_endDay_' + row_id).innerText =
      getDayOfWeek(format_date(d.end));
    document.getElementById('booking_end_' + row_id).innerText = format_date(
      d.end,
      0,
      'DD/MM'
    );
    ...
    document.getElementById('booking_title_' + row_id).innerHTML =
      (services_list.find((item) => item.Service_id === d.Service_id) || {})
        .Title +
      ' (' +
      DaysDifferenceStartEnd(d.start, d.end) +
      ' nuits)';
    document.getElementById('badge_id_' + row_id).innerText = row_id;
    document.getElementById('badge_type_' + row_id).innerText = d.Type_doc;

    document.getElementById('booking_paid_' + row_id).innerText =
      d.encaissement;
    document.getElementById('booking_paid_status_' + row_id).innerHTML =
      d.encaissement >= row_price
        ...
        : d.encaissement < row_price && d.encaissement > 0
  ...
  if (ModalInStack('DetailsEventModal')) {
    ...
    details_paid_div.innerText =
      d.encaissement > 0 ? d.encaissement + ' Fr' : '0';
    if (d.encaissement > 0) {
      let convert_pourc = Math.min(
        Math.round((d.encaissement / row_price) * 10000) / 100,
        100
      );
